...
def employee_edit_data(request,id):
	...
	form = EmployeeCreateForm(request.POST or None,request.FILES or None,instance = employee)
	dataset['form'] = form
	dataset['title'] = 'edit - {0}'.format(employee.id)
	return render(request,'dashboard/employee_create.html',dataset)
...
def dashboard_employee_info(request,id):
	...
	dataset['title'] = 'profile - {0}'.format(employee.id)
...
def dashboard_family_edit(request,id):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			messages.success(request,'Relationship Successfully Updated for {0}'.format(employee.id),extra_tags = 'alert alert-success alert-dismissible show')
			return redirect('dashboard:familycreate')

		else:
			messages.error(request,'Failed to update Relationship for {0}'.format(employee.id),extra_tags = 'alert alert-warning alert-dismissible show')
			return redirect('dashboard:familycreate')
